...
const VehicleManagementPage = () => {
    ...
    const fetchVehicles = async () => {
        try {
            const res = await api.get('/api/vehicles?status=all');
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] p-4">
                    <div className="bg-white rounded p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl border border-gray-200">
                        ...
                        <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest">Kategorija</label>
                                <select
                                    className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 outline-none focus:border-blue-500 transition-all appearance-none"
                                    value={formData.kategorijaId}
                                    onChange={e => setFormData({ ...formData, kategorijaId: parseInt(e.target.value) })}
                                >
                                    <option value="1">EKO</option>
                                    <option value="2">LUX</option>
                                    <option value="3">SUV</option>
                                    <option value="4">GRADSKI</option>
                                    <option value="5">KOMBI</option>
                                </select>
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest">Filijala</label>
                                <select
                                    className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 outline-none focus:border-blue-500 transition-all appearance-none"
                                    value={formData.filijalaId}
                                    onChange={e => setFormData({ ...formData, filijalaId: parseInt(e.target.value) })}
                                >
                                    <option value="1">Beograd (Centar)</option>
